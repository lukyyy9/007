...
const SingleEliminationBracket = ({ 
  ...
  const rounds = Object.keys(winnerBracket).sort((a, b) => parseInt(a) - parseInt(b));
  ...
  const renderRound = (roundNumber, roundIndex) => {
    const roundMatches = winnerBracket[roundNumber] || [];
      ...
      <View key={roundNumber} style={styles.roundContainer}>
        <View style={styles.roundHeader}>
          <Text style={styles.roundTitle}>
            {isLastRound ? '🏆 Final' : `Round ${roundIndex + 1}`}
          </Text>
  ...
  const renderBracketOverview = () => {
    ...
    const totalMatches = Object.values(winnerBracket).flat().length;
    const completedMatches = Object.values(winnerBracket).flat().filter(m => m.status === 'completed').length;
    const activeMatches = Object.values(winnerBracket).flat().filter(m => m.status === 'active').length;

    return (
      <View style={styles.overviewContainer}>
        <Text style={styles.overviewTitle}>Tournament Progress</Text>
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.bracketContainer}>
        {rounds.map((roundNumber, roundIndex) => 
          renderRound(roundNumber, roundIndex)
        )}
      </View>
    </ScrollView>
  );
